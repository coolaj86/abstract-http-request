...
(function (undefined) {
  ...
  ahr = function (options, callback) {
    ahr.http(options).when(function (err, data, response) {
  ...
  function nodeHttpClient(options) {
    ...
    timeoutToken = setTimeout(function () {
      promise.fulfill('timeout', undefined, response);
      cancelled = true;
    }, options.timeout);

    // create Connection, Request
    options.client = http.createClient(options.port, options.host, options.ssl);
    if (options.auth) {
    ...
    request.on('response', function (p_response) {
      ...
      response.on('end', function (chunk) {
        ...
        } else if (response.statusCode && response.statusCode >= 300 && response.statusCode !== 304 && options.followRedirect) {
  ...
  function browserJsonpClient(options) {
    ...
    options.url = extendUrl(options.url, jsonp);
